...
impl dispatch::DeviceInterface for WebDevice {
    ...
    fn set_device_lost_callback(&self, device_lost_callback: dispatch::BoxDeviceLostCallback) {
        let closure = Closure::once_into_js(move |info: JsValue| {
        ...
        // Transfer callback ownership to JS. A stack-owned Closure is invalidated
        // before GPUDevice.lost resolves (upstream 25.0.2 lifetime bug).
        let promise = self.inner.lost();
        let then: js_sys::Function = js_sys::Reflect::get(
            promise.as_ref(), &JsValue::from_str("then")
        ).unwrap().unchecked_into();
        let _ = then.call1(promise.as_ref(), &closure);
    }
