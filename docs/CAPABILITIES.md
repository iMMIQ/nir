...
| 翻译维护 | 源/契约/语义修订、契约摘要、已读语义身份、逐文本状态、显式复核、旧源迁移 | 单模块简中/英文；不迁移跨发行存档，详见 [文本修订](TEXT-REVISIONS.md) |
...
| 语言 | zh-Hans/en 独立 UI/正文偏好与字体计划；候选准备后原子切换；正文下一实例生效，已存在的对白/选项/历史冻结身份 | 单模块的两个正文包启动时均可用；不提供远程语言包流加载、繁简自动回退或多文字系统认证；详见 [语言/字体计划](LOCALE-FONTS.md) |
