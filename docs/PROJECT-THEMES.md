...
`prefetch_media` 默认 `false`，独立控制下一确定 Cue 的图片／音频预取。最多一个批次、32 MiB 增量估算成本，并受现有联合媒体预算限制。遇到分支、调用、目录缺失或准入失败就跳过；不执行剧情。示例显式开启，最小模板保持关闭。准备、取消及验证边界见 [M3 准备调度](M3-PREPARATION.md)。
