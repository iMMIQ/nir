...
尺寸使用视口 CSS 像素。对白 `height` 为 220–320，`padding` 为 12–32，基础 `font_size` 为 18–28。选项 `width` 为 360–680，`item_height` 为 48–72。窄屏会缩窄选项、调整对白尺寸；玩家字号倍率仍作用于对白。参数不能指定动作、写 VM 变量、禁用系统菜单或移除恢复出口。
...
当前主题只支持这两个固定语义槽与封闭的组件清单，不是通用 UI 语言。尚未实现任意每条 Cue 的 Props、主题图片/字体资源、页面组合、标题页替换、主题包继承或第三方组件。主题资源字段会作为未知字段拒绝；标题场景资源仍由既有运行依赖收集器处理。长文本和大量选项的分页/滚动属于后续工作，不能用本次两条选项的响应式验证推断任意内容都不会溢出。
