...
核对日期：2026-09-21。代码基线：早期 `v0.1.0` 开发构建，提交 `4213d8324f717aaf5cf7d8cb77fa6cdf8f238fac`。`docs/specs/` 中六份文件与用户提供的原文件逐字节一致。
...
当前多模块基础已完成 M2.1–M2.3：发布运行时 v2 的模块静态定义、代码、逐语言正文和资源目录可独立驻留、驱逐及重新加载；活动执行与候选恢复通过租约保护，存档原始定义按单元校验，默认开启有界内容预取。接口与验收见 [模块工作流](MODULE-WORKFLOW.md) 和 [分块内容与驻留契约](CONTENT-RESIDENCY.md)。以下仍为原始 v0.1.0 对照，不改写历史结论。
...
交互优化进一步引入精简宿主状态、结构化画面比较、逐项 LRU 文字缓存和可关闭的主线程／渲染分段计时，并为不透明／全透明图片像素增加与原公式逐字节等价的转换快速路径。实测菜单、滚动、启动及预取命中的切换改善，长历史恢复仍有内容对象获取长尾，不能视为已解决。证据见 [交互优化验证](validation/interaction/README.md)。

## 当前能力与后续边界（2026-09-24）

以下是当前分支能力；后文表格保留 v0.1.0 历史，不是当前待办清单。

- **基础协议**：统一有界 inbox、整轮语义预算、请求终态预留、独立暂停 token、取消及代次检查、CI 回归已有实现。
- **作品规模**：多模块链接、逐模块／语言分包、驻留驱逐、分批恢复、内容预取及 32 章容量验证已有实现。
- **M4.1/M4.2**：已补发行校验、stage/晋级/回滚、固定发行入口、按发行隔离的存档、WebGL2 及桌面双浏览器验收，见 [M4](M4-RELEASE.md)。下一主线为 M4.3 作者独立交付。
- **作者流程**：配置来源、封闭主题槽、长文本阅读、文件监听预览、翻译修订及字体处理已有实现。
- **M3 准备路径**：分离网络四槽、音频解码两槽和逐步 owner 上传；支持四级优先级、提升及完成后取消确认。一个确定 Cue 的媒体预取默认关闭，最多 32 MiB 增量估算并受原联合账本约束。接口及限制见 [M3 准备调度](M3-PREPARATION.md)。
- **恢复长尾诊断**：基线 SDK 在两种静态服务下均复现秒级等待；将浏览器临时文件移至 tmpfs，或仅恢复 Chromium 原生共享内存后，30 次恢复 P95 分别约 225／243 ms。硬件测试默认恢复原生共享内存，保留旧配置复现开关。该收益属于测量环境修正，未修改生产缓存或绕过恢复校验。详细证据见 [恢复诊断](validation/interaction/restore-diagnostic.md)。
- **M3 验证**：原生 170 项、宿主 54 项、浏览器 41 项通过；媒体预取五组配对的切换 P95 为 113.3→12.4 ms。完整两小时、1318 轮、131 次设备重建的功能、预算及清理断言通过，WASM 预热后固定为 12.25 MiB。进程 PSS 在前十分钟后仍有约 +26.9 MiB/小时趋势，自动化循环逐渐变慢，物理内存与长期吞吐尚不能视为完全稳定。结果及边界见 [M3 验证](validation/m3/README.md)。

当前准备调度仍是现有资源阶段链，不是任意作者声明的通用 DAG；单 Cue 预测遇到分支、调用或未驻留目录即停止。PNG 解码、字体塑形等原子操作不能中途让出。解码／媒体账本不等于物理内存上限。

产品优先级已重新按“可用、可发行的首版”排序，见 [首版发行差距](V1-RELEASE-GAPS.md)：M4.1/M4.2 已落实发行工具、存档隔离和双渲染后端；接下来完成 M4.3 作者独立交付流程，再安排生产托管及更多设备验收。M3 的 PSS 上移、循环变慢和恢复离群值保留为已知风险；只有证实阻断正常游玩或导致资源耗尽时，才升级为发行阻塞。通用增量构建、更多准备优化与完整线程迁移不再作为默认下一主线。
