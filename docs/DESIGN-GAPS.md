...
1. **定位内容获取与恢复长尾。** 启动并行、压缩传输、章节并发及本轮主线程优化已有真实 GPU 对照。新增计时表明恢复的秒级长尾落在小型内容对象获取区间，额外诊断在缓存命中与禁用缓存两种配置下均复现长尾，应关联浏览器交付、连接、预览服务和系统 I/O，再评估复用已验证内容；不要将偶发等待直接归因为渲染。持续保留同一工作负载样本，耗时硬阈值另行建立。固定次数路线不等于数小时长稳验收。
