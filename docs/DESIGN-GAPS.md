...
后续优先级：先针对 [M3 验证](validation/m3/README.md) 中进程 PSS 小幅上移及循环变慢做独立阶段诊断，补设备／音频存活计数和对照实验；恢复路径保留偶发离群值追踪。再依据实际剩余成本推进增量资源处理、准备配方扩展或线程隔离。Worker／WebGL2、完整语言上下文、优化执行格式、资源 cooking、跨平台／移动验收和 GPU 实际分配观测仍是独立工作，不能由本机 Chromium 通过推导完成。
