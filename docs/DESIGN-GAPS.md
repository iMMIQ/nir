...
当前多模块工作已推进到 M2.1：发布运行时 v2 将模块静态定义、代码、逐语言正文和资源目录拆分为独立对象；标题启动按已保存的语言偏好选择目录与媒体，内容增量安装共享既有正文与指令索引。接口与验收见 [模块工作流](MODULE-WORKFLOW.md) 和 [分块内容与驻留契约](CONTENT-RESIDENCY.md)。以下仍为原始 v0.1.0 对照，不改写历史结论。

## 当前推进顺序（2026-09-23）

1. **M2.2：内容驱逐与重新加载。** 在已固定的 `ContentKey`、原子安装、RAII lease 和按对象 hash 去重的驻留账本上实现回收；有活动引用的内容不可驱逐，历史／检查点仅持有可重新解析的逻辑身份。用跨章节、回退、冷读档、失败与取消测试验证预算约束。
2. **随后：有界预取和准备调度。** 利用模块与语言依赖闭包报告决定预取范围，加入优先级、取消和总工作预算，再测量首句延迟、帧耗时及长时驻留。
3. **继续保留独立验收项。** 物理内存、Worker／WebGL2、优化执行格式、资源 cooking 与增量构建不由 M2.1 的完成推导为已实现。

M2.1 的内容预算是唯一对象的编码字节，默认 16 MiB；单对象／单批次 16 MiB、单批次最多 128 个对象。当前超预算会在提交前失败，自动驱逐属于 M2.2。发行运行时 v2 要求 SDK 与作品一起重建；存档仍要求精确发行匹配。
