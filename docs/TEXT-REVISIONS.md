...
对应 NIR-0006 §8、NIR-0003 §3。本阶段实现作者文本修订、翻译状态和复核流程；独立 UI/正文语言及逐语言字体计划见[语言/字体计划](LOCALE-FONTS.md)。不包含配音目录或跨发行存档迁移。需要从当前源码构建配套 SDK；旧版下载包不会自动升级。
...
当前语言范围为简中/英文、单模块。VoiceKey/SpeakerId、多模块和译文导入导出平台留待后续阶段。文本修订验证见[本阶段报告](validation/text-revisions/README.md)。
