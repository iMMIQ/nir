...
async function poll(){
  if(stopped)return;
  try{
    const r=await fetch('/__nir_dev/status',{cache:'no-store'});if(!r.ok)throw Error('preview unavailable');
    const status=await r.json();
    if(!status.building&&!status.error&&status.release!==initial){location.reload();return;}
