...
export function readPng(png) {
  ...
  const raw=inflateSync(Buffer.concat(chunks)),stride=width*channels,pixels=Buffer.alloc(width*height*channels);let prev=Buffer.alloc(stride),off=0;
...
export function expectPainted(png) {
  const {width,height,channels,pixels}=readPng(png);
  let min=255,max=0;const colors=new Set();
  for(let y=0;y<height;y++)for(let x=0;x<width;x+=3){
    const i=(y*width+x)*channels,light=(pixels[i]+pixels[i+1]+pixels[i+2])/3;
    min=Math.min(min,light);max=Math.max(max,light);colors.add(`${pixels[i]},${pixels[i+1]},${pixels[i+2]}`);
  }
