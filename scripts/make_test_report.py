#!/usr/bin/env python3
"""Freeze successful test evidence beside the human-readable delivery report."""
...
def sanitize(text):
    return text.replace(str(Path.cwd()), "/nir").replace(str(Path.home()), "/build-home")

reports=Path("reports")
browser=json.loads((reports/"browser-results.json").read_text())
assert browser["stats"]["unexpected"]==0 and browser["stats"]["skipped"]==0
native=(reports/"native-tests.log").read_text()
assert "test result: FAILED" not in native
count=sum(map(int,re.findall(r"test result: ok\. (\d+) passed",native)))
assert count>=45
metrics=json.loads((reports/"browser-metrics.json").read_text())
release=json.loads((reports/"final-release-verification.json").read_text())
standalone=json.loads((reports/"standalone-sdk.json").read_text())
assert standalone["status"]=="PASS" and release["status"]=="PASS"
evidence=Path("docs/validation");evidence.mkdir(exist_ok=True)
for name in ["native-tests.log","clippy.log","wasm-clippy.log","sdk-build.log","architecture.log","browser-results.json","browser-run.log","browser-metrics.json","standalone-sdk.json","final-release-verification.json","reproducible-build.log","title.png","dialogue.png","choice-walk.png","ending-walk.png","ending-stay.png","settings.png","history.png","transition.png","touch-dialogue.png"]:
    source = reports/name
    if source.suffix == ".png":
        shutil.copy2(source,evidence/name)
    else:
        (evidence/name).write_text(sanitize(source.read_text()))
shutil.copy2("examples/rain-letters/reports/build.json",evidence/"build.json")
version=subprocess.check_output(["chromium","--version"],text=True).strip()
m=metrics["metrics"]
text=f'''# NIR 首版验收报告

测试日期：{date.today().isoformat()}。测试对象为最终静态目录 `dist/rain-letters-web/`，发行身份：
...
| 验证 | 结果与范围 |
|---|---|
| 原生自动测试 | {count} 项通过，0 失败 |
| 真实浏览器 | {browser["stats"]["expected"]} 项通过，0 跳过、0 失败、0 flaky；用时 {browser["stats"]["duration"]/1000:.1f} 秒 |
...
| 发行完整性 | {release["objects"]} 个对象，逐一验证 SHA-256 与尺寸；真实 WASM 魔数及引用闭包通过 |
...
- 浏览器：{version}，有窗口模式；测试启动参数包含 `--enable-unsafe-webgpu`。
...
| 单次自动通关测量 | 数值 |
|---|---|
| 宿主启动到标题可显示 | {m["titleMs"]:.1f} ms |
| 宿主启动到首句 | {m["firstLineMs"]:.1f} ms，包含测试等待及点击之前的时间 |
| 首次开始输入到首句 | {m["firstLineAfterStartMs"]:.1f} ms |
| 两条路线累计 GPU 帧提交 | {m["frames"]} |
| 标题静置 600 ms | 无新增 GPU 帧提交 |
| 资源准入估算峰值 | {m["peakResidentBytes"]:,} 字节（{m["peakResidentBytes"]/1048576:.2f} MiB） |
| 未压缩运行对象合计 | {release["bytes"]:,} 字节 |
| 音频启动 | {m["audioStarts"]} 次，使用真实 Web Audio 解锁、播放与结束事件 |
...
'''
Path("docs/TEST-REPORT.md").write_text(text)
print(f"Recorded {count} native and {browser['stats']['expected']} browser tests")
